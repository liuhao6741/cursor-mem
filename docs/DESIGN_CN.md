...
- **Agent 可查询**：通过 MCP 工具供 Agent 查询项目历史（memory_search、memory_timeline、memory_get）
- **多项目隔离**：按项目分别存储与注入上下文
...
- **MCP Server**（`cursor_mem.mcp.server`）通过 stdio 与 Cursor 通信，内部使用同一 SQLite 与 search/observation_store/session_store，实现 memory_search、memory_timeline、memory_get。
...
- **配置项**：
  ...
  - `context_budget`：注入上下文的 token 预算（约 4 字符/token）
...
- **builder.build_context(conn, project, config)**：
  ...
  - 组装：标题 + 近期完成会话摘要（_build_summaries_section）+ 最近一场会话的观察列表（_build_observations_section，含去重）+ 项目关键文件列表（_build_files_section，按出现次数排序前 15）+ 页脚（含本地时间）。
...
- 工具：
  - **memory_search(query, project?, type?, limit?)**：FTS 搜 observations + sessions，返回简要列表。
  - **memory_timeline(session_id?, project?, limit?)**：按时间排序的观察列表，时间转为本地显示。
  - **memory_get(ids)**：按 id 取观察详情（含 content、files、本地时间）。
